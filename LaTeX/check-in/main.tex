...
\author[E. Timko, S. Styles, L. Wrubleski]{Edward Timko\inst{1} \and Stephen Styles\inst{2} \and Liam Wrubleski\inst{3}} % (optional, use only with lots of authors)
...
\institute[PIMS] % (optional, but mostly needed)
{
  \inst{1}%
  Dept. of Mathematics\\
...
%     as the expected rate of decay at time $t$ is given by
...
%   Assume the $j$-th member of the chain has decay constant $\lambda_j>0$.
...
% one finds that the evolution of the system is described by the following ODEs:
% $$
...
%     Note that our detector does not measure $N_j(t)$ but instead a sum of expressions of the form $\int_{s_1}^{s_2}\lambda_j N_j(s)ds$,
% where $(s_1,s_2]$ is the interval of time during which the detector is running.
% Nevertheless, it will be in our interest to solve for $N_j$ first.
% The solution of the system are provided by the \textit{Bateman Equation} when $N_1(0)=\cdots=N_\ell(0)=0$:
% $$
